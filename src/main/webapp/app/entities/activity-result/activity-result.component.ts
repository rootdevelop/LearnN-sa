...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.activityResultService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<ActivityResult[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/activity-result'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ActivityResult) {
        return item.id;
    }
